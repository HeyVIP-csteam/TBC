...
export async function saveAccount(env, { username, password, passwordChangedBy, role, officeId, allowedBrands, allowedModules, allowedCountries, fullName, pid, allowedAdminSections, adminSectionEditAccess, canManageAdminAccess, canViewActiveAgents, canViewActivityLogs }) {
  ...
  const account = {
    ...
    // 三国合并（2026-08-20）—— 真实新增字段，不是补丁说明。
    // Same "all" | array shape as allowedBrands, but defaults to []
    // (sees nothing) for brand-new accounts — same reasoning as
    // allowedBrands, not allowedModules: a country-less account
    // should NOT see any data until explicitly granted a country.
    // CRITICAL: unlike allowedBrands/allowedModules, this is NOT
    // bypassed by admin/superadmin rank — see canSeeCountry() in
    // countryAccess.js.
...
// 不改 canSeeBrand/canSeeModule 本身（它们对 admin+ 的"看到一切"逻辑
// 语义不变），国家维度是新增的独立检查点，从 countryAccess.js 转出，
// 这样调用方只需要在原来的 import 行加一个名字，不用另外多 import
// 一个文件。CRITICAL: 这个不会被 admin/superadmin 等级绕过。
