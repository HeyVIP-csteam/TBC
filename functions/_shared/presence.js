...
async function getCurrent(kv, username) {
  const raw = await kv.get(currentKey(username));
  return raw ? JSON.parse(raw) : null;
}

async function getDaily(kv, username, date) {
  const raw = await kv.get(dailyKey(username, date));
 ...
 */
export async function recordHeartbeat(kv, username, { status, device, browser, os }) {
  ...
  const current = await getCurrent(kv, username);
  ...
  if (status === "online") {
    ...
    await kv.put(dailyKey(username, date), JSON.stringify(daily));
  }

  const fresh = { status, lastHeartbeat: nowIso, device, browser, os };
  await kv.put(currentKey(username), JSON.stringify(fresh));
  return fresh;
}
...
export async function getListRow(kv, account) {
  const username = account.username;
  const current = await getCurrent(kv, username);
  const now = Date.now();
  const status = deriveStatus(current, now);
  const daily = await getDaily(kv, username, todayKey());
  return {
    username,
    role: account.role,
...
export async function getLastNDays(kv, username, n = 7) {
  const out = [];
